...
//    }
    ...
    public static EmployeeDTO toEmployeeDTO(EmployeeEntity employeeEntity) {
        ...
        // Fix: Handle projectEntities list properly
        List<ProjectEntity> projectEntities = employeeEntity.getProjectEntities();
        if (projectEntities != null && !projectEntities.isEmpty()) {
            List<ProjectDTO> projectDTOs = new ArrayList<>();
            for (ProjectEntity projectEntity : projectEntities) {
                projectDTOs.add(ProjectMapper.toProjectDTO(projectEntity));
            }
            employeeDTO.setProjectDTOList(projectDTOs);
        } else {
            employeeDTO.setProjectDTOList(new ArrayList<>()); // empty list instead of null
        }
